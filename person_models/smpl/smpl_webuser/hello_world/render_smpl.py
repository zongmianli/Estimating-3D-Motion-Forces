...
'''
# load a pose vecotr
path2pose = '/sequoia/data1/zoli/helpers/sample_poses.npz'
j3d = np.load(path2pose)
#print pose.keys()
pose_all = j3d['sample_poses']
m.pose[:] = pose_all[150]
print m.pose
...
for i in range(24):
    m.pose[:] = np.zeros(m.pose.size)
    for k in range(24):
        m.pose[3*k+x] = np.pi*2    
    #m.pose] = np.zeros(3)
    
    m.pose[i*3+x] = np.pi/6
    cv2.imshow('render_SMPL', rn.r)
    cv2.waitKey(0)
    
    m.pose[i*3+x] = np.pi/3
    cv2.imshow('render_SMPL', rn.r)
